...
from utils import assert_commom, read_add_emp_data, read_query_emp_data, read_modify_emp_data, read_delete_emp_data, \
    DBUtils
...
class TestIHRMEmp(unittest.TestCase):
    ...
    def test01_add_emp(self, username, mobile, successs, code, message, http_code):
        # 调用添加员工接口
        ...
        # 断言
        assert_commom(self, response, http_code, successs, code, message)

    @parameterized.expand(read_query_emp_data)
    def test02_query_emp(self, successs, code, message, http_code):
        # 调用查询员工接口
    ...
    def test03_modify_emp(self, username, successs, code, message, http_code):
        # 调用修改员工接口
        ...
        with DBUtils() as db_utils:
            ...
            sql = "select usernanme from bs_user where id={}".format(app.EMP_ID)
    ...
    def test04_delete_emp(self, successs, code, message, http_code):
        # 调用删除员工接口
