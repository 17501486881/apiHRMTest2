...
def assert_commom(self, response, http_code, success, code, message):
...
def read_add_emp_data():
    ...
    with open(path, mode="r", encoding="utf-8") as f:
        ...
        successs = add_emp_data.get("success")
        code = add_emp_data.get("code")
        message = add_emp_data.get("message")
        http_code = add_emp_data.get("http_code")
        result_list.append((username, mobile, successs, code, message, http_code))
...
def read_query_emp_data():
    ...
    with open(path, mode="r", encoding="utf-8") as f:
        ...
        result_list.append((successs, code, message, http_code))

    print("读取查询员工的数据：", result_list)
    return result_list
...
def read_modify_emp_data():
    ...
    with open(path, mode="r", encoding="utf-8") as f:
        ...
        result_list.append((username, successs, code, message, http_code))

    print("读取修改员工的数据：", result_list)
    return result_list
...
class DBUtils:
    ...
    # __enter__和__exit__是一对一魔法方法，主要要和with结合使用
    # 如：with DBUtils as db:
    # 我们就可以在with的代码块中使用db来执行sql语句，这个db就相当于cursor
    def __enter__(self):
